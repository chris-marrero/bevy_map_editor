...
| ~~`find_layer_index` in `crates/bevy_map_automap/src/apply.rs` is a permanent stub returning `None`~~ | ~~**RESOLVED by T-11 (Barclay, ada4a38).** `find_layer_index` now correctly calls `level.layers.iter().position(|l| l.id == layer_id)`. Fix is in PR #3; pending merge to main.~~ | — |
...
| **[T-12] Input layer combo selection discards `id` in `automap_editor.rs` line 1199** (`let _ = id;`) | **Functional void.** When the user clicks a layer in the input layer combo box, the selected `Layer::id` (UUID) is silently discarded. No state is updated; the combo shows no selection. Automap rules always reference the default (nil) layer ID for input groups. | When the input layer combo wiring task (post-PR #3 merge, Wesley) is completed. |
| **[T-12] Output layer combo selection discards `id` in `automap_editor.rs` line 1217** (`let _ = id;`) | **Functional void.** When the user clicks a layer in the output layer combo box, the selected `Layer::id` is silently discarded. Output alternatives always reference the default (nil) layer ID. | When the output layer combo wiring task (post-PR #3 merge, Wesley) is completed. |
| **[T-12] `make_default_rule()` uses `Uuid::nil()` as placeholder `layer_id` in `automap_editor.rs` line 1239** | Every new rule created via the UI has `InputConditionGroup.layer_id = Uuid::nil()`. No real layer is targeted. The engine will find no matching layer (since `find_layer_index` now resolves correctly), so the rule's input group silently matches nothing. | When the layer combo wiring (T-12 follow-on, Wesley) populates a real default `layer_id` from the project's layer list. |
| **[T-12] `make_default_output_alt()` uses `Uuid::nil()` as placeholder `layer_id` in `automap_editor.rs` line 1254** | Every new output alternative created via the UI has `OutputAlternative.layer_id = Uuid::nil()`. Output writes target no real layer. | When the layer combo wiring (T-12 follow-on, Wesley) populates a real default `layer_id`. |
| **[T-12] `ensure_input_group()` uses `Uuid::nil()` as placeholder `layer_id` in `automap_editor.rs` line 1274** | When an input group is auto-created for a rule with no input groups, it also receives `Uuid::nil()`. Same behavioral impact as `make_default_rule()` stub. This instance was not listed in the original Remmick audit; found during Data's debt review. | When the layer combo wiring (T-12 follow-on, Wesley) is resolved. |
| **[T-13] Layer mapping persistence: UI combo selections not wired to `AutomapEditorState` or `automap_config`** | **In-scope debt, not deferred.** The `automap_config` serde fields (including `layer_id` on `InputConditionGroup` and `OutputAlternative`) are wired to `Project` serialization — so data that gets there is persisted. What is NOT wired: the UI combo selection → `AutomapEditorState` → field update. This is the same path as T-12 stubs. Until wired: user layer assignments made in the UI are never written to `automap_config`, are never saved, and will not survive reload. **Decision (Data, 2026-02-28):** In-scope for automapping sprint. The `TODO(barclay-merge)` labels indicate intentional deferment until PR #3 merged, not out-of-scope deferment. PR #3 is now ready to merge. Wesley must wire the layer selection path after PR #3 merges. | When the T-12 follow-on wiring task (Wesley, post-PR #3 merge) is completed. |

---

## Session Status

**Last updated:** 2026-02-28 (Sprint: Automapping, pre-merge)
