...
use bevy_egui::egui;
use bevy_map_animation::{AnimationDef, LoopMode, SpriteData};
use uuid::Uuid;
...
impl SpriteEditorState {
    ...
    /// Create editor state from sprite data (for editing project-level animation assets)
    pub fn from_sprite_data(sprite_data: SpriteData) -> Self {
        Self {
            instance_id: None,
...
pub fn render_animation_editor(
    ...
    egui::Window::new("Sprite Sheet Editor")
        .open(&mut is_open)
        .resizable(true)
        ...
        .show(ctx, |ui| {
            ...
            if state.preview_playing {
                if let Some(anim_name) = &state.selected_animation {
                    if let Some(anim) = state.sprite_data.animations.get(anim_name) {
                        if !anim.frames.is_empty() {
                            // Get delta time from egui
                            let dt = ui.input(|i| i.stable_dt);
                            state.preview_timer += dt * 1000.0; // Convert to ms
...
fn render_animation_preview(ui: &mut egui::Ui, state: &SpriteEditorState) {
    ...
    // Animation preview - scale up small sprites to at least 128px, cap at 256px
    let min_preview_size = 128.0;
    let max_preview_size = 256.0;

    // Calculate scale to fit within bounds while maintaining aspect ratio
    let scale_up = (min_preview_size / frame_w.max(frame_h)).max(1.0); // Scale up small sprites
    let scale_down = (max_preview_size / (frame_w * scale_up).max(frame_h * scale_up)).min(1.0); // Cap at max
    let final_scale = scale_up * scale_down;

    let display_size = egui::vec2(frame_w * final_scale, frame_h * final_scale);
